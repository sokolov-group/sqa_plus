#    file:  quadratic_residual_2rdm_so.py
#  author:  Eric Neuscamman
#    date:  March 31, 2009
# summary:  Computes tensor contractions for <[[H,A],r]>_(1,2)
#
...
# Define Operators
r1aa = [sqa.term( 1.0, [], [sqa.tensor('r1aa', alphaIndices[16:18], a1sym), sqa.creOp(alphaIndices[16]), sqa.desOp(alphaIndices[17])]), \
        sqa.term(-1.0, [], [sqa.tensor('r1aa', alphaIndices[16:18], a1sym), sqa.creOp(alphaIndices[17]), sqa.desOp(alphaIndices[16])])]
r1bb = [sqa.term( 1.0, [], [sqa.tensor('r1bb', betaIndices[16:18], a1sym), sqa.creOp(betaIndices[16]), sqa.desOp(betaIndices[17])]), \
        sqa.term(-1.0, [], [sqa.tensor('r1bb', betaIndices[16:18], a1sym), sqa.creOp(betaIndices[17]), sqa.desOp(betaIndices[16])])]
r2aaaa = [sqa.term( 1.0, [], [sqa.tensor('r2aaaa', alphaIndices[18:22], a2sym_aaaa), \
                              sqa.creOp(alphaIndices[18]), sqa.creOp(alphaIndices[19]), \
                              sqa.desOp(alphaIndices[21]), sqa.desOp(alphaIndices[20])]), \
          sqa.term(-1.0, [], [sqa.tensor('r2aaaa', alphaIndices[18:22], a2sym_aaaa), \
                              sqa.creOp(alphaIndices[20]), sqa.creOp(alphaIndices[21]), \
                              sqa.desOp(alphaIndices[19]), sqa.desOp(alphaIndices[18])])]
r2bbbb = [sqa.term( 1.0, [], [sqa.tensor('r2bbbb', betaIndices[18:22], a2sym_bbbb), \
                              sqa.creOp(betaIndices[18]), sqa.creOp(betaIndices[19]), \
                              sqa.desOp(betaIndices[21]), sqa.desOp(betaIndices[20])]), \
          sqa.term(-1.0, [], [sqa.tensor('r2bbbb', betaIndices[18:22], a2sym_bbbb), \
                              sqa.creOp(betaIndices[20]), sqa.creOp(betaIndices[21]), \
                              sqa.desOp(betaIndices[19]), sqa.desOp(betaIndices[18])])]
r2baba = [sqa.term( 1.0, [], [sqa.tensor('r2baba', [betaIndices[23],alphaIndices[23],betaIndices[24],alphaIndices[24]], a2sym_baba), \
                              sqa.creOp(betaIndices[23]), sqa.creOp(alphaIndices[23]), \
                              sqa.desOp(alphaIndices[24]),  sqa.desOp(betaIndices[24])]), \
          sqa.term(-1.0, [], [sqa.tensor('r2baba', [betaIndices[23],alphaIndices[23],betaIndices[24],alphaIndices[24]], a2sym_baba), \
                               sqa.creOp(betaIndices[24]),  sqa.creOp(alphaIndices[24]), \
...
d3name = 'd3_dummy'
d4name = 'd4_dummy'
sqa.convert_ops_to_rdms_so(result, d3name, 3)
sqa.convert_ops_to_rdms_so(result, d4name, 4)
sqa.convert_ops_to_rdms_so(result, rdm_dummy_name)
sqa.assign_rdm_types(result, rdm_dummy_name, [d1aa, d1bb, d2aaaa, d2bbbb, d2baba])
print 'density matrix conversion complete at %.3f' %(time.time() - startTime)
print ''

# Decompose 3RDMs
sqa.decomp_3rdms_to_2rdms_so(result, d3name, d1aa, d1bb, d2aaaa, d2bbbb, d2baba)
print '3RDM decomposition complete at %.3f' %(time.time() - startTime)
print ''

# Decompose 4RDMs
sqa.decomp_4rdms_to_2rdms_so(result, d4name, d1aa, d1bb, d2aaaa, d2bbbb, d2baba)
print '4RDM decomposition complete at %.3f' %(time.time() - startTime)
print ''
...
#con_code += 'header end\n'
#con_code += '\n'
#con_code += 'subroutine f_ct_quad_hess_opalg_so\n'
#con_code += '\n'
