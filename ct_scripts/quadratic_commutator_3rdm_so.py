#    file:  quadratic_commutator_3rdm_so.py
#  author:  Eric Neuscamman
#    date:  March 31, 2009
# summary:  Computes tensor contractions for [[H,A],A]_(1,2,(3))
#
...
# Define indices
alphaIndices = [sqa.index('p%i' %i, [sqa.options.alpha_type], True) for i in range(40)]
betaIndices  = [sqa.index('q%i' %i, [sqa.options.beta_type], True) for i in range(40)]
...
d2bbbb = sqa.tensor('d2bbbb', betaIndices[0:4], d2sym_bbbb)
...
h2bbbb = sqa.term(0.25, [], [sqa.tensor('h2bbbb', betaIndices[2:6], d2sym_bbbb), \
...
a2bbbb = [sqa.term( 0.25, [], [sqa.tensor('a2bbbb', betaIndices[10:14], a2sym_bbbb), \
                               sqa.creOp(betaIndices[10]), sqa.creOp(betaIndices[11]), \
                               sqa.desOp(betaIndices[13]), sqa.desOp(betaIndices[12])]), \
          sqa.term(-0.25, [], [sqa.tensor('a2bbbb', betaIndices[10:14], a2sym_bbbb), \
...
# Define Second Amplitudes
b1aa = [sqa.term( 1.0, [], [sqa.tensor('a1aa', alphaIndices[16:18], a1sym), sqa.creOp(alphaIndices[16]), sqa.desOp(alphaIndices[17])]), \
        sqa.term(-1.0, [], [sqa.tensor('a1aa', alphaIndices[16:18], a1sym), sqa.creOp(alphaIndices[17]), sqa.desOp(alphaIndices[16])])]
b1bb = [sqa.term( 1.0, [], [sqa.tensor('a1bb', betaIndices[16:18], a1sym), sqa.creOp(betaIndices[16]), sqa.desOp(betaIndices[17])]), \
        sqa.term(-1.0, [], [sqa.tensor('a1bb', betaIndices[16:18], a1sym), sqa.creOp(betaIndices[17]), sqa.desOp(betaIndices[16])])]
b2aaaa = [sqa.term( 0.25, [], [sqa.tensor('a2aaaa', alphaIndices[18:22], a2sym_aaaa), \
                               sqa.creOp(alphaIndices[18]), sqa.creOp(alphaIndices[19]), \
                               sqa.desOp(alphaIndices[21]), sqa.desOp(alphaIndices[20])]), \
          sqa.term(-0.25, [], [sqa.tensor('a2aaaa', alphaIndices[18:22], a2sym_aaaa), \
                               sqa.creOp(alphaIndices[20]), sqa.creOp(alphaIndices[21]), \
                               sqa.desOp(alphaIndices[19]), sqa.desOp(alphaIndices[18])])]
b2bbbb = [sqa.term( 0.25, [], [sqa.tensor('a2bbbb', betaIndices[18:22], a2sym_bbbb), \
                               sqa.creOp(betaIndices[18]), sqa.creOp(betaIndices[19]), \
                               sqa.desOp(betaIndices[21]), sqa.desOp(betaIndices[20])]), \
          sqa.term(-0.25, [], [sqa.tensor('a2bbbb', betaIndices[18:22], a2sym_bbbb), \
                               sqa.creOp(betaIndices[20]), sqa.creOp(betaIndices[21]), \
                               sqa.desOp(betaIndices[19]), sqa.desOp(betaIndices[18])])]
b2baba = [sqa.term( 1.00, [], [sqa.tensor('a2baba', [betaIndices[22],alphaIndices[22],betaIndices[23],alphaIndices[23]], a2sym_baba), \
                               sqa.creOp(betaIndices[22]), sqa.creOp(alphaIndices[22]), \
                               sqa.desOp(alphaIndices[23]),  sqa.desOp(betaIndices[23])]), \
          sqa.term(-1.00, [], [sqa.tensor('a2baba', [betaIndices[22],alphaIndices[22],betaIndices[23],alphaIndices[23]], a2sym_baba), \
                               sqa.creOp(betaIndices[23]),  sqa.creOp(alphaIndices[23]), \
                               sqa.desOp(alphaIndices[22]), sqa.desOp(betaIndices[22])])]
b = b1aa + b1bb + b2aaaa + b2bbbb + b2baba

# Compute fisrt commutator
comm = sqa.commutator(h,a)
print 'fisrt commutator complete at %.3f' %(time.time() - startTime)
print ''

# Compute second commutator
result = sqa.commutator(comm,b)
print 'second commutator complete at %.3f' %(time.time() - startTime)
print ''
...
# Decompose 4ops
sqa.decomp_4ops_to_2ops_3rdms_so(result, d1aa, d1bb, d2aaaa, d2bbbb, d2baba, d3aaaaaa, d3bbbbbb, d3baabaa, d3abbabb)
print '4-body operator decomposition complete at %.3f' %(time.time() - startTime)
print ''

# Decompose 3ops
sqa.decomp_3ops_to_2ops_3rdms_so(result, d1aa, d1bb, d2aaaa, d2bbbb, d2baba, d3aaaaaa, d3bbbbbb, d3baabaa, d3abbabb)
print '3-body operator decomposition complete at %.3f' %(time.time() - startTime)
print ''
...
#con_code += 'endheader\n'
#con_code += '\n'
#con_code += 'subroutine f_ct_quad_comm_3rdm_opalg_so\n'
#con_code += '\n'
