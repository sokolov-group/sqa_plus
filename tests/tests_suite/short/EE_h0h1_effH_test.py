...
# LHS
l_op  = [sqa_plus.creOp(x), sqa_plus.desOp(a)] # AE
l_ind = 'XA'                                                         

# RHS
r_op  = [sqa_plus.creOp(b), sqa_plus.creOp(u), sqa_plus.desOp(z), sqa_plus.desOp(y)] # AAEA
r_ind = 'YZBU'

# Define Hamiltonian
effH = []
effH = sqa_plus.Heff(0)

term1 = sqa_plus.term(1.0, [], r_op)
term2 = sqa_plus.term(1.0, [], l_op)

term3 = sqa_plus.commutator(effH, term1)
term4 = sqa_plus.commutator(term2, term3)

term5 = sqa_plus.matrixBlock(term4)

sqa_plus.options.genEinsum.remove_core_integrals = False
sqa_plus.options.genEinsum.opt_einsum_terms = False
einsumlist = sqa_plus.genEinsum(term5, 'temp', str(l_ind + r_ind), "")

## Read the reference file:
rfile = open("EE_h0h1_effH_test.ref","r")
