...
#### form in amp, overlap, precond: kuxv, iwyz --> form used here: kwuv, ixyz
#from precond:
#        term_left  = sqa_plus.term(1.0, [], [sqa_plus.creOp(i_alpha), sqa_plus.creOp(u_alpha), sqa_plus.desOp(v_alpha), sqa_plus.desOp(x_alpha)]) #lhs = kuxv
#        term_right = sqa_plus.term(1.0, [], [sqa_plus.creOp(y_alpha), sqa_plus.creOp(z_alpha), sqa_plus.desOp(w_alpha), sqa_plus.desOp(k_alpha)]) #rhs = iwyz
...
if indices_string_right in ['caaa']:
    ...
    X_tensor_abba = [sqa_plus.tensor('X_baab', [i_alpha, x_beta,  y_beta, z_alpha], [])]
...
#                   sqa_plus.term(1.0, [], X_tensor_baab + [sqa_plus.creOp(y_alpha), sqa_plus.creOp(z_beta),  sqa_plus.desOp(x_alpha), sqa_plus.desOp(i_beta)])]
    ...
    X_tensor_baab = [sqa_plus.tensor('X_baab', [i_beta,  j_alpha, a_alpha, x_beta],  [])]
...
print("## Calculating the commutator [H(0), h(1)] ...")
#can set combine = False
terms_commutator = sqa_plus.commutator(terms_Heff, term_left)

print("\n## Calculating [h^(1)\dag, [H(0), h(1)]] ...")
terms_EE_M11 = sqa_plus.commutator(terms_right, terms_commutator)
