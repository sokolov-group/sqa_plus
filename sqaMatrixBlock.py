...
def dummyLabel(_terms, keep_user_defined_dummy_names = True):
    ...
    terms_generator = iter(_terms)

    for _term_ind, _term in enumerate(terms_generator):
        mymap = {}
        ...
        for _tensor_ind, _tensor in enumerate(_term.tensors):
            for _index_ind, _index in enumerate(_tensor.indices):
                ...
                if (index_summed and 
                    ...
                    _index.name = mymap[index_name]
 
        if options.verbose:
            #print("{:} ---> {:}".format(_term_unlabeled, _terms[_term_ind]))
            print("{:} ---> {:}".format(_term_unlabeled, _term))
...
###import psutil, os
###def system_usage():
###    """Returns current system usage: CPU, Memory, and Disk in a tuple (CPU%, Memory_MB, Disk_Usage%)"""
###    
###    # Memory usage
###    process = psutil.Process(os.getpid())
###    memory_usage_mb = process.memory_info().rss / 1024.0 / 1024.0  # Memory in MB
###    
###    # CPU usage
###    cpu_usage_percent = psutil.cpu_percent(interval=0.1)  # Short interval to get instantaneous CPU usage
###    
###    # Disk usage (percentage of disk space used by the system)
###    disk_usage_percent = psutil.disk_usage('/').percent  # Disk usage percentage (for the root directory)
###    
###    return cpu_usage_percent, memory_usage_mb, disk_usage_percent
###
###import gc
###def dummyLabel(_terms, keep_user_defined_dummy_names = True, memory_check = False):
###    "A function to relabel dummy indices."
###
###    # Import options from sqaOptions class
###    user_defined_indices = options.user_defined_indices
...
###
###    if memory_check:
###        cpu, memory, disk = system_usage()
###        print("Initial System Usage: CPU: {:.2f}%, Memory: {:.2f} MB, Disk: {:.2f}%".format(cpu, memory, disk))
###    
...
###
###        if memory_check:
###            print("\nMemory at start of loop _term {}: CPU: {:.2f}%, Memory: {:.2f} MB, Disk: {:.2f}%".format(_term_ind, *system_usage()))
###
###        if keep_user_defined_dummy_names:
###            for reserved_index_name in user_defined_indices:
###                if reserved_index_name in coreInd:
...
###
###                if memory_check:
###                    print("Memory within nested loop: CPU: {:.2f}%, Memory: {:.2f} MB, Disk: {:.2f}%".format(*system_usage()))
###
###                # Decide which new label to assign
###                index_type = _index.indType
###                index_name = _index.name
...
### 
###        if memory_check: 
###            print("After loop _term {}: CPU: {:.2f}%, Memory: {:.2f} MB, Disk: {:.2f}%".format(_term_ind, *system_usage()))
###        if options.verbose:
###            print("{:} ---> {:}".format(_term_unlabeled, _terms[_term_ind]))
###
###    if memory_check:
###        print("Final System Usage: CPU: {:.2f}%, Memory: {:.2f} MB, Disk: {:.2f}%".format(*system_usage()))
###
###    print("Done!")
###    options.print_divider()
###    sys.stdout.flush()
